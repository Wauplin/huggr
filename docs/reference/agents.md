...
Generated CLI and Python crates depend on the exact installed `huggr-toolkit` version from crates.io. A toolkit built from a repository checkout adds a local `[patch.crates-io]` entry for contributor builds; a published toolkit produces registry-only generated crates that do not need a Huggr checkout.
