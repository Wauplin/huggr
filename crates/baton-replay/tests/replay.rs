...
use baton_core::{Command, Event, ModelOutput, OpId, SteerMode, Timestamp, ToolCall, Usage, Value};
use baton_replay::{Inspector, Trace, TraceError, replay, verify};
use serde_json::json;
...
fn replay_round_trips_through_disk() {
    let trace = faithful_trace();
    let dir = std::env::temp_dir().join(format!("baton-replay-disk-{}", std::process::id()));
