...
- `fs_write` creates or appends files, edits an exact text match in place (`fs_edit`), creates one directory, and removes one file or empty directory under `root`; `root = "/"` is an explicit full-disk write grant.
