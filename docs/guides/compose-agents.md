...
- The model ids, token totals, and call totals fold the child in the same way, so the parent `AnswerMeta` lists every effective model used and `huggr stats` totals match the accounting the trace returned (both count children); only cost keeps the own-vs-delegated split.
