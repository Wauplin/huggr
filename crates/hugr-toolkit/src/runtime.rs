...
//! one [`Agent::ask`].
//!
//! External-tool grants (MCP / plugin / agent, §20.3) are not wired here yet —
//! they arrive in ROADMAP T1.5 / T3.8. A definition that declares one gets a
//! warning from [`build_agent`] and the grant is skipped.
...
pub fn build_agent(def: &AgentDefinition) -> Result<(Agent, Vec<String>), RuntimeError> {
    ...
    // Granted tools — sandbox-by-registration (§20.1). Only library grants are
    // wired today; external kinds warn and are skipped.
    for grant in &def.tools {
        match grant.kind {
            ...
            ToolKind::Mcp | ToolKind::Plugin | ToolKind::Agent => {
                warnings.push(format!(
                    "external tool grant `{}` ({:?}) is not wired yet (ROADMAP T1.5/T3.8); skipped",
                    grant.name, grant.kind
                ));
            }
        }
    }
...
"#;
    ...
    #[test]
    fn builds_an_agent_with_library_tools() {
        ...
        let (agent, warnings) = build_agent(&def).unwrap();
    ...
    #[test]
    fn external_tool_grant_warns_and_is_skipped() {
        let src = r#"
[agent]
name = "x"
[models.medium]
...
"#;
        ...
        let (_agent, warnings) = build_agent(&def).unwrap();
        assert!(
            warnings.iter().any(|w| w.contains("external tool grant")),
            "{warnings:?}"
        );
    }
}
