...
`fs_read` and `fs_write` are each jailed to one or more canonicalized roots, declared per grant:
...
Grant several roots with `roots` (an array of path strings or `{ name, path }` tables) instead of a single `root`. Each root is its own canonicalized jail; with more than one, tool paths are addressed as `<root-name>/<path>` and a search or listing with no path spans every root. Names default to the final path component and must be unique. The jails never merge, so a traversal out of one root cannot reach another.
