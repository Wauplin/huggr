...
//! built as its own binary, depending on nothing from Hugr — adds a working
...
use hugr_core::{ModelOutput, ModelRequest, ModelSelector, Record, ToolCall, Usage, Value};
use hugr_host::policy::AllowAll;
use hugr_host::{Engine, Frontend, ModelAdapter, ModelSink, SubprocessPlugin};

/// Path to the example plugin binary (built by cargo for this crate's tests).
const PLUGIN_BIN: &str = env!("CARGO_BIN_EXE_hugr_example_plugin");
...
async fn subprocess_transport_describe_and_invoke() {
    use hugr_host::{PluginSink, PluginTransport};
...
async fn agent_calls_plugin_tool_through_the_engine() {
    ...
    let caps = hugr_host::plugins::load(plugin)
        .await
    ...
    let model = MockModel::new([
        ModelOutput::tool_calls(vec![ToolCall::new(
            "call-1",
            "uppercase",
            serde_json::json!({ "text": "hugr plugins work" }),
        )]),
    ...
    assert_eq!(
        tool_result["text"], "HUGR PLUGINS WORK",
        "the plugin computed the result: {tool_result}"
    );
}

/// Sanity: the plugin never links Hugr — it is a real out-of-tree tool. (This
...
fn plugin_binary_is_standalone() {
    ...
    let _ = Value::Null; // keep the hugr-core import meaningful
}
