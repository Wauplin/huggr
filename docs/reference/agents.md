...
- **`fs_read`:** root-jailed read-only family including list, literal search, regular-expression grep, glob, reads, and outlines. Accepts one `root` or several named `roots`; with more than one, paths are addressed as `<root-name>/<path>` and searches without a path span every root.
- **`fs_write`:** root-jailed file creation/replacement/append, exact-match single-file editing, single-directory creation, and non-recursive removal; accepts one `root` or several named `roots`. Also grants the `fs_read` family on the same root(s) (write implies read), unless a separate `fs_read` grant owns the read jail.
