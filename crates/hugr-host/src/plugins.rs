...
use hugr_plugin_abi::{PluginError, PluginSink, PluginTransport, SubprocessPlugin};
...
///
/// ```no_run
/// # async fn run() -> Result<(), hugr_plugin_abi::PluginError> {
/// let tools = hugr_host::plugins::load_subprocess("my-plugin", ["--flag"]).await?;
/// # let _ = tools; Ok(()) }
/// ```
