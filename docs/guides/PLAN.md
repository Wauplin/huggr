...
1. **Serving and consuming MCP** (`15-mcp.md`). Exposing a built agent as an MCP server with `--mcp-serve`, the `ask` and `feedback` tools, and granting external MCP servers with `[tools.mcp.<name>]`.
